...
class TiltState:
    HORIZONTAL, VERTICAL_LEFT, VERTICAL_RIGHT = Range(-0.25, 0.25), Range(1.4, 1.8), Range(-1.8,-1.4)
    ...
    def Update(self, values):
        ...
        if TiltState.HORIZONTAL.Belongs(self.Rotation):
            if (self.State == TiltState.VERTICAL_LEFT or self.State == TiltState.VERTICAL_RIGHT) and (time.time() - self.StartTime) >= TiltState.VALID_TILT_TIME:
	            print 'all found'
        ...
        if (TiltState.VERTICAL_LEFT.Belongs(self.Rotation) or TiltState.VERTICAL_RIGHT.Belongs(self.Rotation)) and self.State == TiltState.HORIZONTAL:
            # change state
            self.StartTime = time.time()
            print 'vertical'
