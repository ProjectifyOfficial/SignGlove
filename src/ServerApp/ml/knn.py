...
class Point:
    ...
    def __mul__(self, other):
        return sum([self.coords[i] * other.coords[i]
                    for i in range(len(self.coords))])
...
class KNNClassifier (Classifier):
    ...
    def Vote(self, p, K=1, f=lambda x: x + 1, append_to_sample_data=False):
        ...
        for symbol in keys:
            if closePoints[candidateSymbol[0]
                           ] == closePoints[symbol] and candidateSymbol[0] != symbol:
                #K_prime = int(f(K))
...
class KMeansClassifier (Classifier):  # some stack overflow stuff
    ...
    def kmeans(data, k, c):
        ...
        iterations = 0
        while not (
            KMeansClassifier.has_converged(
                centroids,
                old_centroids,
                iterations)):
            iterations += 1
            ...
            clusters = KMeansClassifier.dist(
                data, centroids, clusters, norm=Norms.EuclideanNorm)  # generalize norm
        ...
        for cluster in clusters:
            print("Cluster with a size of " +
                  str(len(cluster)) + " starts here:")
...
class SymbolManager:
    ...
    def Update(self, values):
        ...
        # else:
            ...
            # identifiedGesture = candidate_class[0]                      # the
            # only element of actives array
